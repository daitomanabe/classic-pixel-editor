...
mirror_hits="$(find . -path './.git' -prune -o -path './.build' -prune -o -path './.ralph' -prune -o -path './ralph.yml' -prune -o -path './PROMPT.md' -prune -o -path './SPEC.md' -prune -o -path './CLEANROOM.md' -prune -o -path './README.md' -prune -o -path './ROADMAP.md' -prune -o -path './docs/feature-research.md' -prune -o -path './.github/pull_request_template.md' -prune -o -type f -print0 | xargs -0 grep -Ein 'github\.com/.+(photoshop|photo-shop)|raw\.githubusercontent\.com/.+(photoshop|photo-shop)|agdm/.+1\.0\.1' || true)"
